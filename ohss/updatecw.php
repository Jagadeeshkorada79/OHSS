...
?>
 ...
 <body>
  ...
  <h1 style="text-align: center;">CHIEFWARDEN UPDATE FORM</h1>
 <div style="height: 630px; width: 60%; border:2px solid #365978; margin-top: 10px;margin-left: 350px;">
    <?php
 include 'db_con.php';
  $sql="SELECT * FROM cfdata WHERE uemail='$userid'";
...
include 'db_con.php';
 if (isset($_POST['update'])) {
  ...
  $sql="UPDATE cfdata SET uname='$_POST[uname]',unum='$_POST[unum]',ushift='$_POST[ushift]' WHERE uemail='$id' ";
    $result=mysqli_query($conn,$sql);

    $sqlp="SELECT * FROM logcf WHERE umail='$id'";

    $resultp=mysqli_query($conn,$sqlp);
    move_uploaded_file($_FILES["image"]["tmp_name"],"uploads/" . $_FILES["image"]["name"]);

    $sqlu = "UPDATE cfdata SET img ='$filename' WHERE uemail = '$id' ";
    ...
    if (mysqli_num_rows($resultp) === 1) {
      ...
      if ($rows['password'] === $pass) {
           $sqlU="UPDATE logcf SET password='$npass' WHERE umail='$id' ";
