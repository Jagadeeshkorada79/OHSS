...
<div id="id2" class="modal">
   <div class="modal-content animate" style="border: 5px solid green;">
    ...
    <tr class="tr2"><td class="td2">Reason:</td><td><input type="text" name="reason"value="<?php echo $rowt['reason']?>"></td></tr>
     ...
     <input type="submit"  style="margin-left:150px" name="denied" value="OUT CAMPUS">
    <input type="submit"  style="margin-left:50px" name="approve" value="IN CAMPUS">
<?php
                     }
  ...
  if (isset($_POST['approve'])) {
    $sby=$rowc['uname'];
     $query ="UPDATE `sdata` SET `status` = 'IN CAMPUS'  WHERE `id` = $id";
     $query_run=mysqli_query($conn,$query);
  }
   if (isset($_POST['denied'])) {
    $sby=$rowc['uname'];
     $query ="UPDATE `sdata` SET `status` = 'OUT CAMPUS'  WHERE `id` = $id";
     $query_run=mysqli_query($conn,$query);
  }
?>
<?php
