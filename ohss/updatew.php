...
?>
 ...
 <body>
  ...
  <h1 style="text-align: center;">WARDEN UPDATE FORM</h1>
 <div style="height: 630px; width: 60%; border:2px solid #365978; margin-top: 10px;margin-left: 350px;">
    <?php
 include 'db_con.php';
  $sql="SELECT * FROM wdata WHERE uemail='$userid'";
...
<center><a href="warden.php"><button>Go back</button></a></center>
</body>
</html>
<?php
include 'db_con.php';
 if (isset($_POST['update'])) {
  ...
  $sql="UPDATE wdata SET uname='$_POST[uname]',unum='$_POST[unum]',ushift='$_POST[ushift]' WHERE uemail='$id'";
    $result=mysqli_query($conn,$sql);

    $sqlp="SELECT * FROM logw WHERE umail='$id'";

    $resultp=mysqli_query($conn,$sqlp);
    move_uploaded_file($_FILES["image"]["tmp_name"],"uploads/" . $_FILES["image"]["name"]);

    $sqlu = "UPDATE wdata SET img ='$filename' WHERE uemail = '$id' ";
    ...
    if (mysqli_num_rows($resultp) === 1) {
      ...
      if ($rows['password'] === $pass) {
           $sqlU="UPDATE logw SET password='$npass' WHERE umail='$id' ";
